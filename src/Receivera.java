...
//            destination = session.createTopic("testTopic"); 
            consumer = session.createConsumer(destination,"client='A'");  
...
//	            	System.out.println(u.getUsermessage());
	            	 try {
						...
						text.setJMSCorrelationID(text.getJMSMessageID());
						// 回复一个消息 
						 Queue  queue = new ActiveMQQueue("testQueue"); 
						 MessageProducer producer = session.createProducer(queue);  
		                  
		                    Message replyMessage = session.createTextMessage("回复生产者来自a。。。。"+text.getText());  
